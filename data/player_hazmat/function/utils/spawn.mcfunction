...
$execute rotated ~ 0 run function animated_java:player_hazmat/summon {args:{'variant':'$(variant)'}}


#> ID system between the model and the player
scoreboard players operation @s player_hazmat.ID = .global player_hazmat.ID
scoreboard players operation @e[tag=player_hazmat.new] player_hazmat.ID = .global player_hazmat.ID
