...
# Summon + ID System
execute if score weapons.animated weapon.config matches 1 as @s[tag=player_slim] rotated ~ 0 run function animated_java:hands_tools_polaroid/summon {args:{'variant':'slim'}}
execute if score weapons.animated weapon.config matches 1 as @s[tag=player_default] rotated ~ 0 run function animated_java:hands_tools_polaroid/summon {args:{'variant':'default'}}
