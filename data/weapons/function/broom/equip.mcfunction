...
# Summon + ID System
execute as @s[tag=player_slim] rotated ~ 0 run function animated_java:hands_weapons_broom/summon {args:{'variant':'slim'}}
execute as @s[tag=player_default] rotated ~ 0 run function animated_java:hands_weapons_broom/summon {args:{'variant':'default'}}
...
# Set Weapon Id
scoreboard players set @s equipped.weapon.ID 2